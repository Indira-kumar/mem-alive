...
class SemanticStore(Store):
    ...
    async def remember(self, namespace: str, fact: str, metadata: dict):
        ...
        await self._supersede_similar_memory(
            namespace=namespace, fact_vector=fact_vector, metadata=metadata
        )
        id = str(uuid.uuid4())
        memory = Memory(
            id=id,
    ...
    async def _supersede_similar_memory(
        self, namespace: str, fact_vector: list[float], metadata: dict
    ):
        search_results = await self._db.search(
            namespace=namespace,
            vector=fact_vector,
            top_k=self._contradiction_check_k,
            metadata=metadata,
        )
        search_results = [
            r
            for r in search_results
            if r.memory.superseded is False and r.memory.memory_type == "semantic"
        ]
        ...
        if best_match.score > self._contradiction_threshold:
            updated_memory = replace(
                best_match.memory, superseded=True, updated_at=datetime.now(timezone.utc)
            )
