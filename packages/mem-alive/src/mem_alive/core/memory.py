...
class Memory:
    ...
    async def recall(
        self, namespace: str, search_query: str, metadata: dict, top_k: int|None = None, memory_type:str|None = None
    ) -> list[MemoryRecord] | None:
        metadata = metadata or {}
        if memory_type:
            return await self._stores[memory_type].recall(namespace=namespace, search_query=search_query, metadata=metadata,
                                             top_k=top_k)
        results = await asyncio.gather(*(store.recall(namespace, search_query, metadata, top_k) 
